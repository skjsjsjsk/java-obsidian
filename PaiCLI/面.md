...
- 我现在的系统还不支持断电续跑
- 第一层: 
	- 取消: 运行中取消, LLM开始前, 返回后, 工具使用前后都会检查用户是否按了ESC或者输入/cancel来取消, 避免了无意义的循环
- 第二层就是文件修改恢复: 靠Side-Git回滚快照
...
- 第三层是后台任务恢复: RUNNING任务重启后重新入队
	- 我把当前任务的持久化状态保存到SQLit里面, 这里面有两个重要字段 status 和 prompt, status代表该任务当前的状态, prompt就是执行该任务的自然语言的指令. 
	- 如果你准备执行某个任务了, 把它提交后, status变成ENQUEUED, 当Worker领取任务时就会变RUNNING
# 当前项目中是否有hook呢? 它有什么作用? 
