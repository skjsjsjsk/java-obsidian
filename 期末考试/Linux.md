...
7. `grep '^\*' file 查找以*打头的行`, 单引号意思是把引号中的内容交给grep去处理, ^在正则表达式里代表它接下来的字符必须在一行的开头, 然后是*, 因为 * 特殊的, 所以要加转义字符/, /* 表示 * ; sort 默认升序, sort -r 可以降序, `sort -n -t : -k 2 facebook.txt, 它的意思是-n, 按数值排序, -t指定分割符为: , 然后文本内容被分割开了, -k 2指定了排序的字段为第2列`; uniq只处理
