...
-  InnoDB表的数据存在哪
	- 这个问题考的是对InnoDB存储结构的基本认知。
	- InnoDB用的是聚簇索引，**整张表的数据就存在主键索引的B+树叶子节点上**。不是单独存一份数据文件，索引和数据是在一起的。
	- 如果建表的时候没有显式指定主键，InnoDB会找一个非空唯一索引来充当聚簇索引。如果连这个也没有，InnoDB会自动生成一个6字节的隐藏rowid作为聚簇索引。