...
第三类是 Agent 工具结果缓存，比如 `agent:tool-result:v1:{toolName}:{hash}`，目前只缓存 `search_knowledge` 和 `knowledge_stats(知识库统计工具)`，分别是 10 分钟和 60 秒；hash 里包含 userId、toolName 和规范化参数，避免跨用户串数据。
