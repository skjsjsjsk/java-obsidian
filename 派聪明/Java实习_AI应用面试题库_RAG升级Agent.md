...
**话术：**  
我实现的 ReAct 流程可以简单理解成循环：用户问题进来后，先构造 system prompt 和工具列表，让 DeepSeek 判断下一步是直接回答还是调用工具。如果返回 `tool_calls`，后端解析函数名和参数，先做参数校验，再执行对应工具，比如 ES 检索、文件搜索或外部 API。工具执行结果会包装成 Observation，再作为 tool message 放回上下文，继续请求模型。这个循环会有最大 step 限制，比如最多 3 次工具调用，避免模型一直绕圈。每一步我会记录 stepId、toolName、参数、耗时、状态和错误信息。这里关键是：模型只负责“决定调用什么工具和生成参数”，真正执行工具的是 Java 后端。总结来说，我做的是一个受控的 ReAct 执行器，不是让模型无限自由发挥。
...
**话术：**  
我在 prompt 里没有只写“你可以检索知识库”，而是把工具使用条件写得比较明确。比如用户问项目文档、上传文件、制度内容、历史知识时，必须优先调用 `knowledge_search`；如果是通用闲聊，就不要调用工具。工具参数里我会定义 `query`、`topK`、`searchMode`、`docScope` 这些字段，`searchMode` 可以是 `keyword`、`vector` 或 `hybrid`。如果用 DeepSeek 的 tools，我会把这些写进 JSON Schema，并让 `tool_choice` 默认是 `auto`。对于明显需要查文档的问题，我会把 `tool_choice` 指定为具体 function，避免模型跳过检索直接回答。我测试时发现，工具 description 写得太泛会导致模型乱选工具，所以后来我把“什么时候用、什么时候不用、结果为空怎么办”都写进了系统约束。总结就是，Prompt 不是文案，而是工具路由规则的一部分。
...
**话术：**  
我的 ES 检索主要是混合检索思路。关键词部分用 Query DSL，比如对标题、正文 chunk 做 `match` 或 `multi_match`；语义部分用向量字段做 `knn`，根据用户问题生成 query embedding 后查相似 chunk。最终会把两路结果合并，按分数、文档权重、是否命中标题等因素做简单排序。ES 文档里每个 chunk 都带元数据，比如 `docId`、`chunkId`、页码、文件名、权限标签，这样可以在 DSL 里加 filter，避免越权检索。刚开始我只做向量检索，发现一些专有名词、编号、接口名召回不稳定，所以后来加了关键词 DSL 作为补充。总结来说，向量检索解决语义相似，关键词检索解决精确词和实体命中，Agent 拿到的是融合后的证据。
...
**话术：**  
我处理检索为空时，不是直接让模型编答案，而是把它当成一个可控的失败分支。第一步会判断 ES 返回为空、最高分低于阈值、或者没有可引用 chunk，这些都算检索不可靠。然后 Agent 会进入 query rewrite，比如去掉口语词，提取核心实体，把长问题拆成关键词。第二步会切换检索策略，比如从 `knn` 向量检索切到关键词 DSL，或者从 keyword 切到 hybrid，并适当扩大 `topK`。如果两轮仍然失败，就不再继续调用工具，而是返回“知识库没有找到依据”，让用户补充文档或更明确的关键词。我测试时发现，失败后如果没有 step 限制，Agent 会重复问 ES 类似的问题，所以我加了最大重试次数和 Redis 短期去重。总结就是，自我纠错必须有边界。
...
**话术：**  
我分块时没有简单按固定字符硬切，因为这样容易把一个概念切断。我的思路是先按标题、段落、换行做初步切分，再控制每个 chunk 的长度，比如几百到一千字左右，保留一定 overlap，避免上下文断裂。每个 chunk 会生成 embedding，然后写入 ES 的向量字段，同时保存原文、页码、文件名、docId、chunkId。这样 Agent 回答时可以把引用定位到具体片段，而不是只说“来自某个文档”。我测试时遇到过一个问题：chunk 太短，召回很多碎片，模型难总结；chunk 太长，又浪费 Token，还容易把无关内容塞进上下文。所以我后来用一些测试问题去调 chunk 大小和 topK。总结来说，分块是 RAG 到 Agent 的底层质量基础，不能只靠 Prompt 补救。
...
**话术：**  
我做了一个比较朴素但有效的限制：最终回答里的引用必须来自 ES 返回的 chunkId 或 docId，不能让模型自己生成文件名。后端把检索结果结构化传给模型，比如每条证据带 `chunkId`、标题、页码和内容摘要，Prompt 里要求答案只能引用这些 ID。如果检索结果为空或者分数低，就不能输出确定结论。生成后我还会做一次简单校验，看答案里的引用 ID 是否存在于本轮上下文。这个不是特别复杂的算法，但能解决模型“看起来很有依据，其实引用不存在”的问题。我测试时确实遇到过模型编造文档名，所以后来把引用从自然语言改成结构化证据列表。总结就是，引用可信度要靠后端校验，不能完全相信模型自觉。
