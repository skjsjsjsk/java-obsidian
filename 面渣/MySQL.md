...
- 为什么InnoDB要用B+树做索引:  B+树是一种自平衡的多路查找树, 它的每个节点可以有n个子节点. 这就使得它的树高很矮, 通常3,4层就可以存储几千万条数据, 有效降低了查询的IO次数;  同时, B+树的叶子节点只存储数据, 非叶子节点只存储键值对和指针, 同时叶子节点之间使用双向链表来连接 ,使得查询效率稳定高效. 而且可以在非叶子节点上存储更多的键值对, 同时范围查询时, 不需要回树, 多次遍历, 可以直接在叶子节点之间顺序访问
	- 对比其他数据结构: 
		- 哈希表: 只能等值查询
