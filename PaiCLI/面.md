...
- 然后就会把conversationHistory, 工具Schema传给LLM, LLM返回tool_calls, Agent检测需要调用工具, 会先把这条assistant tool_call消息加入到conversationHistory里面, 然后再解析tool_calls, 把它返回的工具转为一个内部ToolInvocation, 交给toolRegistry去执行. 
