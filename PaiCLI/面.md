...
- 然后就是发送给LLM前会压缩. ReAct主循环每次调用模型前都会估算当前的对话历史的token, 如果接近了90%, 那么就把早期历史进行Map-Reduce压缩, 把这些长对话分段摘要(Map 5个一批), 再合并为一个总摘要(Reduce), 用摘要替代原始的对话历史来释放空间. 同时保留最近3轮的上下文作为短期记忆
...
- 关于具体的压缩就是我保留最近的三轮对话作为短期记忆, 然后把更久远的对话历史去进行压缩, 通过得到它们各自的use message, 使得分割点落到边界上, 防止把同一个工具的调用和它的调用结果切分开导致下一轮对话时LLM无法理解
...
# 18. 遇到的问题, 如何解决
- 关于问题的话, 我碰到过一个很典型的问题, 就是一开始我在Memory那一块已经做了有关Memory的压缩机制, 但是我在测试长对话, 多轮工具调用的时候, 仍然会很快的达到模型上下文窗口的上限导致停止. 
- 所以我就去看日志, 发现了Memory的压缩确实生效了, 所以我就想应该是还有一部分没有被我压缩导致的. 我就去我的代码, 发现了记忆确实会被压缩传入system prompt中, 但是真正传给LLM的是conersationHistory, 里面不只有system pr

我在 PaiCLI 里遇到过一个比较典型的问题：项目已经有 Memory 压缩机制，但长对话、多轮工具调用之后，LLM 请求还是会越来越大，甚至接近模型上下文窗口。(这个问题比较隐蔽，因为表面上看短期记忆已经压缩了，但实际发给模型的 messages 并没有变短。)

然后我就想是不是少压缩了什么, 但是日志里可以看到Memory确实已经压缩过了, 所有我就给每次LLM调用前的token占用进行了分类, 分为了systemTokens, userTokens, assiantant, toolMessageTokens, 用来统计发给模型的不同部分的内容的token数量. 具体定位时就发现了, 短期记忆的token数压缩后确实下降了, 但是一到了下一轮的请求, userTokens/ assistantTokens/ toolMessageTokens 仍然会持续增长, 尤其是多轮工具调用后, toolMessageTokens增长很快. 可见, 真正发送给LLM的conversationHistory其实并没有被压缩

后来定位发现，项目里有两套上下文数据。第一套是 MemoryManager 里的 shortTermMemory，它主要用于记忆检索和上下文注入；第二套是 Agent 主循环里真正传给 LLM 的 conversationHistory。当时压缩只压了短期记忆，但 Agent.run() 每轮调用模型时，实际传的是 conversationHistory，所以 shortTermMemory 压缩并不能减少 HTTP 请求里的 messages token。

解决方案是单独做了一层 ConversationHistoryCompactor。在每次 ReAct 循环调用 LLM 前，先估算当前 conversationHistory token 是否超过阈值。阈值来自 ContextProfile，默认是模型上下文窗口的 90%。如果超过，就保留最近 3 轮完整上下文，把更早的历史消息交给 LLM 摘要，然后重建 conversationHistory。

这里最关键的设计点是：**不能随便从中间截断消息**。因为 ReAct 里有 assistant 的 tool_calls 和后续 role 为 tool 的工具结果，它们是成对的。如果从中间切断，下一轮请求可能不符合 OpenAI tool calling 协议。所以我选择按 user message 边界切分，保留最近几轮完整交互，把旧历史压成一条：

`user: [已压缩的历史对话摘要] ... assistant: 好的，我已了解之前的上下文，请继续。`

这样既保留了历史任务目标、关键工具结果和未完成事项，又不会破坏 tool_call / tool_result 的协议完整性。

最终效果上，解决了“Memory 显示压缩了，但实际请求仍然变长”的问题。现在长任务在调用 LLM 前会真正压缩 conversationHistory，降低上下文爆窗风险。这里如果写到简历里，需要补充真实指标，比如：

`将长任务场景下单次请求 tokens 从 X 降到 Y，压缩后平均下降 Z%`

或者：

`在自建长对话 benchmark 中，压缩触发后请求上下文稳定控制在窗口 90% 以下`
