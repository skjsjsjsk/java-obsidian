...
- 然后我就去定位问题了, 发现我一开始想的是LLM调用的时候从短期记忆去重建消息, 这样可以用的上它的压缩机制了, 但是实际上LLM调用时传给Agent的是一个conversationHistory, 它会在问题第一次进来的时候构建上下文, 检索短期和长期记忆, 重建system prompt, 替代conversationHistory[0]的system prompt, 然后
