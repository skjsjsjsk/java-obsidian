...
然后就是生成护栏: 首先就是system prompt里面, 明确知识库相关问题必须调用 `search_knowledge`, 强制基于检索内容回答; 如果现有证据不能够支持回答, 那么就拒绝回答, 不要瞎编; 还有就是检索出来的chunk会被编号, 方面模型输出后溯源; 最后就是规定了系统规则优先级最高, 用户的注入不能够改变它
