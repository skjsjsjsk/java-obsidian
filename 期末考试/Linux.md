...
7. `grep '^\*' file 查找以*打头的行`, 单引号意思是把引号中的内容交给grep去处理, ^在正则表达式里代表它接下来的字符必须在一行的开头, 然后是*, 因为 * 特殊的, 所以要加转义字符/, /* 表示 * ; sort 默认升序, sort -r 可以降序, `sort -n -t : -k 2 facebook.txt, 它的意思是-n, 按数值排序, -t指定分割符为: , 然后文本内容被分割开了, -k 2指定了排序的字段为第2列`; uniq只处理相邻的重复行, 如果不相邻 就不能去除, 所以一般使用会先sort再uniq
8. cp, 如果是目录, 要加 -r, -i可以交互式, 比如 `cp  mfile  /home/mengqc/exam1` 你要复制文件, 如果exam1不存在, 那么就创建文件重命名; 如果存在并且有内容, 那么会静默覆盖, 如果有-i参数会提示你是否有覆盖, 输入y就覆盖; cp复制目录要加-r
9. rm默认不能删除目录, -r递归式的删除指定目录及其下属; -i交互式; -f强制删除不提示
...
11. wc, 输出格式为 `行数 字数 字节数 文件名`, -l 行数, -w 单词数, -c 字节数, -m 字符数
12. paste, 按列合并文件, 字面意思, -d用来指定分隔符, 默认为空格, -s串行处理
13. find, 
    ![](assets/Linux/file-20260630175534654.png)
14. > , 重定向输出, 会覆盖; >> 这个是追加, <<这个的意思是从下一行开始读取输入, 直到遇到指定的结束标记, 比如`cat << EOF > file.txt` 就是重定向输出到file.txt文件中, 直到碰到EOF才停止
15. Linux目录的作用
	...
	5. /home: 普通用户主目录的基础目录
16. 绝对路径和相对路径: 绝对路径: 从根目录 / 开始, 相对路径从当前工作目录开始
...
19. 硬链接和软连接: 
	...
	2. 软链接: 通过ln -s, 软链接里面保存的是原文件的地址, 原文件被删了也就不能用了, 它有自己的inode, 可以链接目录, 可以跨文件系统
20. chmod: `chmod a+x ex1, chmod o-r file, chmod u=rw,g=r,o= file`
21. umask: 用来设置新建文件或目录文件的默认权限掩码. `目录最大权限：777, 普通文件最大权限：666, umask则是去掉权限, 比如umask 022, 创建目录默认权限为: 777-022=755, 创建文件默认权限为: 666-022=644`
22. chgrp 和 chown: 
	1. chgrp: 修改文件所属用户组
...
23. ps: Linux标准的进程查看工具
24. kill: kill本质是向进程发送信号. kill -9是强制结束
25. df: 查看文件系统磁盘使用清空
26. tar: 是打包命令. -c: 创建文档; -x: 解包; -t: 查看包内容; -f: 指定文件名; -z: 使用gzip; -v: 显示过程  `c/x/t三者只能选一个, -f后面紧跟文件名`
	1. 例子: 
		1. 打包并 gzip 压缩:  tar -zcvf archive.tar.gz dir
		2. 查看 tar.gz 内容:   tar -ztvf archive.tar.gz
		3. 解压 tar.gz:          tar -zxvf archive.tar.gz
27. sed: 默认不修改原文件, -i才直接修改
    ![300](assets/Linux/file-20260630222741940.png)
    
29. vi的工作模式: 进入vi后默认命令模式, 按i`(光标前插入)`/a`(光标后追加)`/o`(当前行下新开一行)`进入插入模式; 插入模式按ESC回到命令模式; 命令模式按 : 进入Ex模式(转义/末行模式) 
	1. 退出: :w保存;  :q退出, 要求文件未修改;  ZZ和 :wq 类似, 不过ZZ如果未修改文件的话不会更新文件时间戳;  :x类似ZZ;  :q! 不保存退出
	2. 光标移动: 左: h;; 下: j 上: k; 右: l
30. gcc: -g加入调试信息供gdb使用; -c只生成 .o , 不链接. 
    **预处理->编译->汇编->链接**
...
31. shell脚本的第一行一定是: #!/bin/bash
	...
	6. continue 2, 表示到外层第2层循环, 进入下一次循环; break 2跳出循环
```
