...
- ==三级缓存来解决循环依赖问题==: 
	...
	- 举个例子: 
		...
		- 为什么不是二级缓存而是三级缓存: 主要为了解决AOP代理的问题, 如果没有AOP代理，两级缓存就够了
			- 还是AB循环依赖. 如果是二级缓存, 当A实例化以后, 为了B能够注入A, 就要把A放到二级缓存里面, 而二级缓存用来存储原始对象或者代理对象, 那么就要在实例化后, 属性注入前去决定来生成哪种对象去存入, 破坏了Bean的生命周期
			- 如果三级缓存, 三级缓存里面的工厂去判断是生成一个原对象还是代理对象, 这样保证了不会出现两个Bean对象
		- 哪种循环依赖无法解决: 
