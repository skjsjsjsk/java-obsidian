...
在这个期间, 如果模型返回了一个tool_calls, 就说明它要调用工具了, 比如 `search_knowledge` 检索工具, 这时候就会给前端推一个 `tool_call` 的状态事件, 比如 "正在检索知识库" 这样子. 工具执行完后就会生成一个Observation放回模型上下文, 继续让模型生成后续.
...
**考察点：** 是否知道响应式项目里最常见的坑是乱用阻塞调用。
 ...
 对于上面那些, 我把ReAct放到了一个单独的工作线程里面去跑, 像PDF解析这种走Kafka去异步消费这样子规避阻塞.
### 13. 多个工具能不能并发调用？怎么保证顺序？

**考察点：** 是否会区分可并发任务和有依赖任务。

现在暂时不能. 因为我当前项目中的ReAct主流程基本上是串行去执行工具的. 因为很多步骤都有推理依赖, 比如先检索知识库, 再根据拿到的 Observation 再决定接下来要不要继续检索或者总结. 并发同时跑的话可能会让模型拿到互相冲突的上下文. 后续的话如果工具多了, 比如有多个只读工具像知识库统计, 检索之类的, 这样就可以并发去调用它们.
### 14. WebSocket 断开了怎么办？
...
超时这块，当前代码不是在每一个工具层单独包响应式超时，而是在 `ChatHandler` 里对一次生成做 120 秒截止. 等待当前这轮 ReAct 结果时会每隔200ms去看用户是否点击停止或者超时，超时或用户点停止就调用 `StreamHandle.cancel()` , 取消上游流，后续 token 不再处理/推送。

限流和 Token 不足会通过配额服务、错误事件和 WebSocket 返回给前端。

模型不调用工具的问题，我现在主要靠 prompt 强约束：除严格白名单外，问题默认要先 `search_knowledge`；`tool_choice` 当前仍是 `auto`，还没有落地强制工具调用。
...
### 22.1 该项目咋样部署的护栏, 避免了幻觉和攻击等导致的大模型违规

我主要做的是工程侧的护栏.
首先就是检索护栏: 混合检索做了前置过滤, 防止检索到不在权限内的文档. 
然后就是生成护栏: 
	首先就是system prompt里面, 明确知识库相关问题必须调用 `search_knowledge`, 强制基于检索内容回答; 如果现有证据不能够支持回答, 那么就拒绝回答, 不要瞎编; 
	然后就是检索出来的chunk会被编号, 方面模型输出后溯源; 
	还有就是规定了系统规则优先级最高, 用户的注入不能够改变它(防prompt注入)
	最后就是做了工具白名单, 模型返回tool_calls后会查看它是否在白名单中, 防止随便调用和未知工具
## 六、评估、成本与线上质量
