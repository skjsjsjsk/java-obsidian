...
-  下一步就开始构建OKHttp请求. 就是首先它会把对话历史和工具Schema通过bulitRequestBody(message, tools)这个方法把它们转化为OpenAI-compatible格式的JSON, 得到JSON后就构建Http请求
- LLM, LLM返回tool_calls, Agent检测需要调用工具, 会先把这条assistant tool_call消息加入到conversationHistory里面, 然后再解析tool_calls, 把它返回的工具转为一个内部ToolInvocation, 交给toolRegistry去执行. 
