...
- ==三级缓存来解决循环依赖问题==: 
	...
	- 举个例子: 
		...
		3. B注入A: B要注入A, 那么获取A, 先从一级缓存获取, 不存在完整A, 二级缓存, 不存在早期A, 三级缓存, 找到了A的对象工厂, 此时调用工厂的`getObject()`方法得到了A的早期引用, 把 A 的早期引用放入二级缓存，并从三级缓存移除
		4. B初始化完成: B属性注入完成, B完成了初始化, 将B放到一级缓存里面
		5. A重新注入B并完成初始化: 和B一样, 完成初始化, A从二级缓存删除, 放到一级缓存里面
		6. 最后A,B都完成了初始化且都在一级缓存里面
	
		- 为什么不是二级缓存而是三级缓存: 主要为了解决AOP代理的问题, 如果没有AOP代理，两级缓存就够了
			- 还是AB循环依赖. 如果是二级缓存, 当A实例化以后, 为了B能够注入A, 就要把A放到二级缓存里面, 那么这时候就要决定是放原对象还是代理对象了.  如果放原对象, 那么在后置处理后就会给一级缓存里放一个代理对象, 此时导致了有两个Bean对象.  放代理对象, 就会破坏Bean的生命周期, 代理对象应该在后置处理阶段生成, 而不是实例化完之后就生成
			- 如果三级缓存, 三季缓存里面的工厂去判断是生成一个原对象还是代理对象, 这样保证了不会出现两个Bean对象
		- 哪种循环依赖无法解决: 
			- 通过构造器进行注入的, 它要求对象被new出来的那一刻依赖对象也已经准备好了, 但是三级缓存是对象被new出来之后才被放进缓存的
...
- ==关于AOP==:AOP（面向切面编程）把跟业务逻辑无关但是被业务模块共同调用的逻辑或者责任（比如事务、日志、权限校验）封装起来, 减少重复代码, 在运行时通过动态代理织入到目标方法。**实现系统功能和核心业务逻辑的解耦。**
...
- ==说一下IOC与DI==: 
	...
	- DI就是把对象创建依赖所需要的对象注入到对象中去. 也是实现IOC的具体手段, 有三种:
...
- ==Spring事务==: Spring事务分为编程式事务和声明式事务, 编程式事务可以自己控制事务的开启, 结束, 回滚等等, 比较灵活, 而声明式事务就是在方法上加上一个注解, @Transactional, Spring会自动帮我管理该事务的整个生命周期
	...
	- 事务传播机制: (生效的前提就是通过代理对象调用)就是多个事务方法互相调用时, 事务如何传播;  Spring中有7种事务传播机制, 其中REQUIRED是默认的传播行为
		...
		- **REQUIRES_NEW**: 不管有没有事务都新建一个事务. 如果存在事务, 就将该事务挂起
		- NOT_SUPPORTED: 以非事务方式执行操作。如果当前存在事务，把当前事务挂起。
		- **NESTED**: 如果存在事务, 就在当前事务中创建一个嵌套事务(内层事务依赖于外层事务, 外回滚内也回滚). 如果没有, 就与REQUIRED类似, 嵌套事务回滚不影响外层事务, 通常只回滚到保存点
...
- ==SpringBoot自动装配==: 在 `@SpringBootApplication` 中有三个核心注解, 分别是`@SpringBootConfiguration(标记这是个SpringBoot配置类)`, `@EnableAutoConfiguration(启动自动装配)` 和 `@ComponentScan(启用组件扫描)`.       
	- 其中 `@EnableAutoConfiguration` 是自动装配的关键, 它通过 `@Import` 导入了`AutoConfigurationImportSelector` 类, 这个类从Jar 包的 META-INF/spring.factories文件读取, 这些文件记录了配置类的全类名, 通过全类名找到对应的配置类, 配置类里定义了Bean, 再根据这些Bean的**条件注解**判断它们是否需要被加载到Spring容器中
	- 条件注解: 
