...
- 第三块就是系统提示词方面
	- [面](面.md#^9e8f1f)
# 9. 你的Agent loop咋设计的呢?(HitlToolRegistry, 继承ToolRegistry)
...
- 然后就进入循环了, 会先检查是否有LSP注入, conversationHistory是否需要压缩, 然后通过AgentBudget判断是否超token预算, 超最大轮数限制, 出现重复工具调用, 都没有的话就会通过getToolDefinitions方法从toolRegistry中拿到所有的工具的Schema. 
- 然后就会把conversationHistory, 工具Schema传给LLMClient 的chat方法. 
-  下一步就开始构建OKHttp请求. 具体就是它会把对话历史和工具Schema通过bulitRequestBody(message, tools)这个方法把它们转化为OpenAI-compatible格式的JSON, 得到JSON后就构建OKHttp请求
- 接下来就是发送请求并且解析SSE流. 具体就是请求发送后, Client会逐行读取SSE的`data` 事件, 每一个事件里面都有一个delta字段, 它里面有三类内容, 分别是reasoning_content, content, tool_calls. 前两个字段会通过 `StreamRenderer` 实时渲染给用户, tool_calls会按index去累计id, name, arguments, 直到完整
- 解析完后LLMClient会返回一个 `ChatReponse` , 这里面就有content, reasoningContent, tool_calls以及inputTokens, outputTokens和cacheInputTokens. 
- 然后就回到了Agent loop主循环, 会检测LLM是否返回tool_calls, 返回, 会先把这条assistant tool_call消息加入到conversationHistory里面, 然后再解析tool_calls, 把它返回的工具转为一个内部ToolInvocation, 交给toolRegistry去执行. 
- 如果是单工具, 直接执行, 多工具可以并行执行, 并发度最多为4. 结果按tool_call的顺序返回. 工具执行的结果会作为一个 `rool=tool` 的消息回灌到conversationHistory, 通过tool_call_id 和 assistant tool_call 里的id来对齐, 然后继续下一轮循环.
- 直到模型不再返回tool_calls, 认为任务完成, Agent会把最终结果写入历史, 记录token的使用, 更新短期记忆和状态栏, 然后把最终答案返回给CLI
...
# 10. System Prompt是咋设计的呢?

^9e8f1f

- 我的Prompt是动态拼装的, 而且是分层进行的. 总的来说, 整个提示词被分成了8层.
- 第一层是base.md, 这里面定义了PaiCLI的身份, 语言要求, 还有工具的列表, 工具的使用说明等等
...
# 13. 上下文压缩咋做的呢(conversationHistory压缩), 解决“实际发给 LLM 的 messages 超窗口”的问题?
- 第一就是90%的阈值: 就是我会在每次LLM调用之前检查是否需要进行上下文压缩. 具体来说的话就是:
	- 我会先算出有效的上下文窗口为当前使用模型上下文窗口的90%, 比如200k的窗口有180000token有效, 
	- 然后在有效窗口的基础上预留了13000token的安全缓冲, 所以最终用来压缩的阈值为167000token, 大概为原窗口的83.5%
	- 我会每次调用LLM前就会检查当前的对话历史占用的token数, 如果大于该阈值, 那么就压缩, 否则继续
- 第二就是如果没有来得及压缩的话, 在有效上下文窗口的基础上预留3000token的缓冲, 会在上下文超过180000-3000token(大约88.5%)时的时候直接拒绝发送请求, 防止收到413错误, 浪费API的调用
## 如何进行压缩
...
- 我现在的系统还不支持断点续跑
- 但是有一个Side-Git回滚快照
	- 每次执行任务前都会创建一个 `pre-turn快照`, 任务结束/异常/取消后会异步创建一个 `post-turn快照`.
	- 它底层使用的一个独立的Side-Git仓库记录项目文件的状态, 不污染用户自己 `.git` 历史
	- 如果要恢复可以用 `/restore n` 恢复到最近第n轮的pre-turn commit记录; 或者可以使用 `revert_turn` 这个工具. 
	- **重要的是**它在恢复前还会创建一个 `pre-restore` 快照, 恢复操作不和预期还能回来重试
# 16. 异步后台任务咋设计的
- 我有一个命令, 可以通过 `/task add` 这个命令提交任务去异步执行
- 具体来说的话就是: 
	- 任务通过命令提交以后进入SQLite排队, 后台有一个Worker Pool不断去领取任务执行, 各自独立运行
		- 任务进入队列的status为enqueued, 领取任务后更新为running, 返回结果会更新为completed/failed/canceled
	- worker领取任务的原子性通过事务机制来保证. 同时更新状态从 `enqueued` -> `running`用乐观锁来保证防止重复领取任务
# 17. 当前项目中是否有hook呢? 它有什么作用? 
- 在我的Pai-CLI中, 它会在你执行写文文件操作后自动执行post-edit hook, 这个就是LSP诊断注入.
- 诊断模块会自动对修改的代码进行语法分析, 收集错误和警告信息, 在下一轮LLM请求前把它作为 `user message` 注入到 `conversationHistory` 中, LLM看到后会自动进行修复