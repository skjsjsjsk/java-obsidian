...
# 18. 遇到的问题, 如何解决
- 关于问题的话, 我碰到过一个很典型的问题, 就是一开始项目已经有 Memory 压缩机制, 但是我在测试长对话, 多轮工具调用的时候, 仍然会很快的达到模型上下文窗口的上限导致停止. 
- 后来我详细去代码中定位发现, 项目里是有两种上下文数据的. 第一种就是短期记忆, 用于记忆检索和上下文的注入; 第二种就是Agent每次循环真正传给LLM的conversationHistory. 当时我只压缩了短期记忆, 但是对于conversationHistory, 
